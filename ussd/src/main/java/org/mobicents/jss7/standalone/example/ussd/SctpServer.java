		...
		if (logger.isDebugEnabled()) {
			try {
				logger.debug(String.format("onProcessUnstructuredSSRequestIndication for DialogId=%d. Ussd String=%s",
                        procUnstrReqInd.getMAPDialog().getLocalDialogId(), procUnstrReqInd.getUSSDString().getString(Charset.defaultCharset())));
		...
		if (logger.isDebugEnabled()) {
			try {
				logger.debug(String.format("onUnstructuredSSResponseIndication for DialogId=%d Ussd String=%s", unstrResInd
                        .getMAPDialog().getLocalDialogId(), unstrResInd.getUSSDString().getString(Charset.defaultCharset())));
			} catch (MAPException e) {
				e.printStackTrace();
			}
		}
